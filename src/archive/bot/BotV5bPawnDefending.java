...
public class BotV5bPawnDefending implements Bot {
    ...
    private MutableGameStatePawnDefending[][] pools = new MutableGameStatePawnDefending[10][218];
    ...
    public BotV5bPawnDefending(boolean log, boolean print) {
        this.log = log;
        this.print = print;
    }
    ...
    private int evaluate(MutableGameStatePawnDefending state, boolean partial) {
    ...
    private int iterativeDeepening(GameState state, double allottedTime) {
        ...
        score = 0;
        pools = new MutableGameStatePawnDefending[10][218];
        while (watch.getElapsedTimeMillis() / 1000d < allottedTime) {
            if (!thread.isAlive()) {
                ...
                thread = new Thread(() -> {
                    try {
                        pools = new MutableGameStatePawnDefending[10][218];
                        move[0] = minimaxMove(state.asMutablePawnDefending(), depth[0], state.isWhiteMove(),
                                move[0]);
                        depth[0]++;
    ...
    public int iterativeDeepening(GameState state, int maxDepth) {
        ...
        while (depth <= maxDepth) {
            ...
            move = minimaxMove(state.asMutablePawnDefending(), depth, state.isWhiteMove(), move);
            depth++;
        }
        if (log) {
            if (!new File("depths.txt").exists()) {
    ...
    private int minimaxMove(MutableGameStatePawnDefending state, int depth, boolean isMaximizing,
                            int bestMoveIdx) {
        assert depth > 0;
        state.computeMoves();
        ...
        final MutableGameStatePawnDefending[] nextStates;
        final boolean sortMoves = depth > 2;
        if (sortMoves) {
            moveSearchOrder = new Integer[state.getMoveCount()];
            nextStates = new MutableGameStatePawnDefending[state.getMoveCount()];
        ...
        MutableGameStatePawnDefending nextState;
        if (depth + 1 >= pools.length)
            pools = new MutableGameStatePawnDefending[Math.max(depth, pools.length * 2) + 1][218];
        for (int i = 0; i < state.getMoveCount(); i++) {
    ...
    private int minimaxScore(MutableGameStatePawnDefending state, int depth, int maxDepth, boolean isMaximizing,
                             int alpha, int beta) {
        ...
        int bestScore = isMaximizing ? Integer.MIN_VALUE / 2 : Integer.MAX_VALUE / 2;
        int score;
        MutableGameStatePawnDefending nextState = null;
        for (int i = 0; i < state.getMoveCount(); i++) {
            if (sortMoves) {
