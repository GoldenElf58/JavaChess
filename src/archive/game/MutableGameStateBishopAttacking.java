...
public class MutableGameStateBishopAttacking {
    ...
    private byte blackRooks;
    private byte whiteRooks;
    ...
    private byte lastMutatingMoveIdx;
    private byte lastMutatingPieceTaken;
    ...
    private MutableGameStateBishopAttacking() {
    }

    private MutableGameStateBishopAttacking init(byte[] board, boolean whiteQueen, boolean whiteKing,
    ...
    public MutableGameStateBishopAttacking(byte[] board, boolean whiteQueen, boolean whiteKing, boolean blackQueen,
    ...
    public MutableGameStateBishopAttacking makeMove(int moveIdx) {
        int idx = moveIdx * 3;
        lastMutatingMoveIdx = (byte) moveIdx;
        return makeMove(moves[idx], moves[idx + 1], moves[idx + 2]);
    }

    public MutableGameStateBishopAttacking makeMove(byte move_0, byte move_1, byte move_2) {
        return loadMoveTo(new MutableGameStateBishopAttacking(), move_0, move_1, move_2);
    }

    public MutableGameStateBishopAttacking loadMoveTo(MutableGameStateBishopAttacking child, int moveIdx) {
        int idx = moveIdx * 3;
        lastMutatingMoveIdx = (byte) moveIdx;
        return loadMoveTo(child, moves[idx], moves[idx + 1], moves[idx + 2]);
    }

    public MutableGameStateBishopAttacking loadMoveTo(MutableGameStateBishopAttacking child, byte move_0, byte move_1, byte move_2) {
        byte[] newBoard = board;
        ...
        byte blackRooks = this.blackRooks;
        byte whiteRooks = this.whiteRooks;
        byte whiteQueens = this.whiteQueens;
        byte blackQueens = this.blackQueens;
        ...
        if (move_0 == -1) {
            ...
            return child.init(newBoard, whiteQueen, whiteKing, blackQueen, blackKing,
                    ...
                    whiteRooks, blackQueens, whiteQueens, otherPieces,
        ...
        if (move_0 == -2) {
            ...
            else if (move_2 == 4) whiteRooks++;
            else if (move_2 == -4) blackRooks++;
            else if (move_2 == 5) whiteQueens++;
            else if (move_2 == -5) blackQueens++;
            else throw new IllegalStateException("Pawns cannot promote to piece " + move_2);
            newBoard[move_1] = 0;
            lastMutatingPieceTaken = 0;
            return child.init(newBoard, whiteQueen, whiteKing, blackQueen, blackKing,
        ...
        if (move_0 <= -4) {
            ...
            byte captured = board[move_2];
            if (captured == 2) whiteKnights--;
            else if (captured == 3) whiteBishops--;
            else if (captured == -2) blackKnights--;
            else if (captured == -3) blackBishops--;
            else if (captured == -4) blackRooks--;
            else if (captured == 4) whiteRooks--;
            else if (captured == -5) blackQueens--;
            else if (captured == 5) whiteQueens--;
            else otherPieces--;
            if (move_0 == -4) {
                ...
                if (color == 1) whiteRooks++;
                else blackRooks++;
            } else if (move_0 == -7) {
                if (color == 1) whiteQueens++;
                else blackQueens++;
            } else throw new IllegalStateException("Unknown piece type for promotion taking. " +
                    "move_0 = " + move_0);
            return child.init(newBoard, whiteQueen, whiteKing, blackQueen, blackKing,
                    (byte) -1, halfMoves + 1, (byte) (halfMoveClock + 1), !whiteMove, null, false,
                    (byte) 0, blackKnights, whiteKnights, blackBishops, whiteBishops, blackRooks,
    ...
    public boolean isInProgress() {
        ...
        if (otherPieces + whiteQueens + blackQueens + whiteRooks + blackRooks == 0) {
    ...
    private boolean isCentral16Square(byte idx) {
        byte idxMod8 = (byte) (idx & 7);
        byte idxDiv8 = (byte) (idx / 8);
        return (idxMod8 > 1 && idxMod8 < 6) && (idxDiv8 > 1 && idxDiv8 < 6);
    }

    private void addSlidingMovesBonus(byte i, byte direction1, byte direction2) {
        byte idxMod8 = (byte) (i & 7);
        byte idxDiv8 = (byte) (i / 8);
        for (byte j = 1; j < 8; j++) {
            byte target = (byte) (i + direction1 * j + direction2 * j * 8);
            ...
            byte targetPieceType = (byte) (board[target] * color);
            if (targetPieceType == 0) {
                if (color == 1) whiteBonus += isCentral16Square(target) ? 5 : 2;
                else blackBonus += isCentral16Square(target) ? 5 : 2;
                addMoveSlot(i, target);
                continue;
            }
            if (targetPieceType < 0) {
    ...
    private void addMovesForBishop(byte i) {
        addSlidingMovesBonus(i, (byte) 1, (byte) 1);
        addSlidingMovesBonus(i, (byte) 1, (byte) -1);
        addSlidingMovesBonus(i, (byte) -1, (byte) 1);
        addSlidingMovesBonus(i, (byte) -1, (byte) -1);
    }
