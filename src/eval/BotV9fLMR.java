...
public class BotV9fLMR implements Bot {
    ...
    public BotV9fLMR(boolean log, boolean print) {
        this.log = log;
        this.print = print;
    }
    ...
    private int minimaxScore(MutableGameState state, int depth, int maxDepth, boolean isMaximizing,
        ...
        for (int i = -2; i < state.getMoveCount(); i++) {
            ...
            else if (maxDepth - depth <= 3 && i > max(3, sqrt(state.getMoveCount()) - 1) &&
                    !nextState.inCheck() && nextState.numPieces() == state.numPieces())
